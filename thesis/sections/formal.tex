...
While there are a number of system properties that can be expressed using trace properties, secrecy and authentication are the most common use case as far as security protocols are concerned.
Secrecy can be expressed using the following lemma, stating that for any secret action $x$ at timepoint $i$, there exsists no timepoint $j$ at which the adversary has knowledge about $x$.
The very last line is added to ensure satisfiability of the lemma in the face of potential compromise of an honest participant's key.
Beyond this simple assumption, \textsc{Tamarin} further provides support for perfect forward secrecy.
...
\textsc{Tamarin} offers support for the following four types of authentication.
\textbf{Aliveness} merely mandates that if participant $a$ completes a protocol run at timepoint $i$, there exists a participant $b$ that has previously run the protocol.
Intuitively, this is a rather weak premise that is not useful for the \gls{prins} protocol.
...
\textbf{Weak agreement} expresses that if participant $a$ completes a protocol run at timepoint $i$, there exists a participant $b$ that has previously run the protocol with $a$.
...
\textbf{Non-injective agreement} describes that if participant $a$ completes a protocol run at timepoint $i$, there exists a participant $b$ that has previously run the protocol with $a$ and both participants agreed on a message term $t$.
...
Furthermore, there exisits an injective mapping between the number of protocol executions by $a$ and $b$.
That is, both participants execute the protocol together each time, not more and not less often.
...
This type of check is particularly revelent for proving privacy properties, for example in e-voting protocols.

\subsubsection{Featureset \& Limitations}

security properties:
\begin{enumerate}
    \item reachability (secrecy)
    \item trace properties (authentication)
    \begin{enumerate}
        \item aliveness
        \item weak agreement
...
\noindent
A \textit{Horn clause} is a clause containing at most one positive literal, for example:
...
Ssecurity properties:
\begin{enumerate}
    \item reachability (secrecy)
    \item correspondence (authentication)
    \begin{enumerate}
        \item weak agreement
        \item injective agreement
    \end{enumerate}
    ...
    \begin{enumerate}
        \item strong secrecy
        \item offline guessing
        \item process instances
        \item processes
    \end{enumerate}
\end{enumerate}
