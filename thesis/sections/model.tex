...
In contrast to N32-f, N32-c communication utilizes private channels.
Since communication follows a client-server model, each \gls{sepp} maintains a dedicated channel for sending messages.
Logically, these represent end-to-end channels between two communicating \glspl{sepp} guaranteeing both confidentiality and integrity.
...
Channels {\sffamily err\_a} and {\sffamily err\_b} are used internally to the two peers, \gls{sepp}~A and \gls{sepp}~B, to pass information about errors in N32-f messages from the receiving N32-f processes to the those handling error signaling via N32-c.
Channels {\sffamily plmn\_a} and {\sffamily plmn\_b} connect the \gls{nf} in each of the networks to its local \gls{sepp}.
...
(2)~The N32-f context information, which contains information about the validity of session keys and whether or not \gls{tls} shall be used to transport N32-f messages.
...
This particular constructor takes four information elements of type {\sffamily bitstring}, {\sffamily bitstring}, {\sffamily mac}, and {\sffamily bitstring} and produces a single {\sffamily ipxmod} element.
...
Listing~\ref{lst:functions} shows an example of custom functions used in the \gls{prins} model.
By default, they are available to all protocol participants, including the adversary.
Below functions for the derivation of N32-f context ID, master key, session key, \gls{aes} initialization vector and nonce as well as the application of \gls{json} patches to an incoming N32-f message are one-way functions.
...
The second deconstructor {\sffamily getMess} depicted in the listing below is provided to allow the adversary to retrieve signed message contents without possessing the public key for validation, as suggested by the official \textsc{ProVerif} user's manual (c.f. \cite{blanchet2020proverif}, p.~14).
...
The receiving N32-c handshake process in listing~\ref{lst:n32c-handshake-recv} is structured similarly to the sending one, with a few key distinctions.
...
Listing~\ref{lst:n32c-error-send} shows the sending \gls{sepp}'s part of error signaling process carried out over N32-c, the details of which are described not in the security specification \gls{ts} 33.501, but \gls{ts} 29.573 (c.f. \cite{3gpp.29.573}, p.~18).
...
How the contents of this response are determined by the \gls{sepp} responding to such error signaling is shown in the next listing~\ref{lst:n32c-error-recv}.\pagebreak
...
The \gls{sepp} initially awaits a modified \gls{prins} message with a destination address that matches its own.
...
Lines 1114-1116 show how the message operations (i.e. the modifications itself), the \gls{ipx} provider ID, the \gls{jwe} tag, and a \gls{jws} signature of the complete message are combined to form this patch.
...
Afterwards, the \gls{ipx} provider's patch is combined with the original message contents in lines 1130-1138 to form a modified \gls{prins} message of type {\sffamily modprins}.
