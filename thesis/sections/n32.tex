...
Intuitively, one can see that this overlaps with the premises of the Dolev-Yao model: All signaling messages are intercepted by an \gls{ipx} provider (the potential adversary) and all messages received are sent by such an entity.
This constellation, in which different external parties are expected to perform cetain changes that cannot be validated or traced with the existing security protocols is one of the motivating factors for the definition of \gls{prins}.
